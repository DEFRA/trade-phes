...
public class PlantsCommodityPopulator implements ApplicationFormFieldPopulator {

  private final CommodityPlantsStringGenerator commodityPlantsStringGenerator;
  ...
  private void populate(final List<Commodity> commodities, final Map<String, String> fields) {
    ...
    final ArrayList<String> commoditiesList =
        commodities.stream()
            .map(commodity -> (CommodityPlants) commodity)
            .map(
                commodity ->
                  count.getAsInt() + ") " + commodityPlantsStringGenerator.generate(commodity)
