...
public class UsedMachineryCommodityPopulator implements ApplicationFormFieldPopulator {

  private final CommodityInfoService commodityInfoService;

  private static final String COMMODITY_DETAIL_TEMPLATE = "%s) %s, %s, %s, %s %s";
  ...
  private void populate(final List<Commodity> commodities, final Map<String, String> fields) {
    ...
    final ArrayList<String> commoditiesList =
        commodities.stream()
            .map(commodity -> (CommodityMachinery) commodity)
            .map(
                commodityMachinery ->
                    String.format(
                        COMMODITY_DETAIL_TEMPLATE,
                        count.getAsInt(),
                        commodityMachinery.getMachineryType(),
                        commodityMachinery.getMake(),
                        commodityMachinery.getModel(),
                        commodityMachinery.getUniqueId(),
