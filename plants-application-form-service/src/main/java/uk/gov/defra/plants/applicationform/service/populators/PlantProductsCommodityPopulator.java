...
public class PlantProductsCommodityPopulator implements ApplicationFormFieldPopulator {

  private final CommodityPlantProductsStringGenerator commodityPlantProductsStringGenerator;
  ...
  private void populate(final List<Commodity> commodities, final Map<String, String> fields) {
    ...
    final ArrayList<String> commoditiesList =
        commodities.stream()
            .map(commodity -> (CommodityPlantProducts) commodity)
            .map(
                commodity ->
                    count.getAsInt()
                        + ") "
                        + commodityPlantProductsStringGenerator.generate(commodity)
